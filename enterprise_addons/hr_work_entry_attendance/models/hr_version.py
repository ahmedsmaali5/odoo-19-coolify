...
class HrVersion(models.Model):
    ...
    def _get_overtime_intervals(self, start_dt, end_dt):
        ...
        domain = Domain([("manual_duration", ">", 0)])
        emp_domain = Domain.FALSE

        for version in self:
            tz = timezone(version.employee_id.tz or version.employee_id.resource_id.tz or "UTC")
            start_localized = start_naive.astimezone(tz)
            end_localized = end_naive.astimezone(tz)
            emp_domain |= Domain(
                [
                    ("employee_id", "=", version.employee_id.ids),
                    ("date", "<=", end_localized.date()),
                    ("date", ">=", start_localized.date()),
                ]
            )
